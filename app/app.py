...
while True:
    ...
    if event == sg.WIN_CLOSED or event == "Exit":
        ...
        conCodigos = pd.merge(consolidado,movimientosAux, on=['Concepto', 'Mayor2','codMov'], how='left').fillna(valorDefecto)
        ##   las apropiaciones
        ####### eso
        
        for i in range(len(conCodigos)):
            objeto =conCodigos.loc[i,"Glosa"]+str(conCodigos.loc[i,"Detalle"])
            #print(i, "=",objeto)
            buscareis = re.findall(r'(%s)'%apachurrado,objeto,flags=re.IGNORECASE+re.MULTILINE)
            ...
            if conCodigos.loc[i,"Tipo"] in["D","V"]:
                if conCodigos.loc[i,"Concepto"] == "Ingreso":
                    conCodigos.loc[i,"codigoBC"]="AJUSTE POR ARBITRAJE DE SALDOS (BCI32)"
                else:
                    conCodigos.loc[i,"codigoBC"]="AJUSTE POR ARBITRAJE DE SALDOS (BCE29)"
                    
            if conCodigos.loc[i,"Mayor2"] ==498:
                if conCodigos.loc[i,"Concepto"] == "Ingreso":
                    conCodigos.loc[i,"codigoBC"]="ABONOS TRANSITORIOS (BCI31)"
            
            if conCodigos.loc[i,"codigoPropio"]==valorDefecto:
                #print ("este ", este)
                #print(conCodigos.loc[i,"codigoPropio"])
                conCodigos.loc[i,"codigoPropio"]=', '.join(map(str,unasola))
